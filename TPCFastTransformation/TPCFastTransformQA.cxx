...
int TPCFastTransformQA::doQA( Long_t TimeStamp )
{
  ...
  if(1){ 
	...
	AliHLTTPCGeometry::Sector2Slice( slice, slicerow, iSec, iRow);
	for( float pad=0.5; pad<nPads; pad+=1.){
	  for( float time =0; time < lastTimeBin; time += timeStep ){
	    Int_t is[]={iSec};
	    double orig[3]={static_cast<Double_t>(iRow),pad,time};
	    float fast[3];
	    origTransform->Transform(orig,is,0,1);
...
int TPCFastTransformQA::doQASRS( Long_t TimeStamp )
{
  const char *fileName = "fastTransformQASRS.root";
  ...
  TPCFastTransformSRS fastTransform;
  TPCFastTransformManager man;
  
  man.createSRS( fastTransform, origTransform, TimeStamp );   
  ...
  // measure execution time
  {
    ...
    cout<<"nCalls2 = "<<nCalls2<<endl;
    cout<<"Orig transformation\t: "<< timer1.RealTime()*1.e9/nCalls1<<" ns / call"<<endl;
    cout<<"Fast transformation SRS\t: "<< timer2.RealTime()*1.e9/nCalls2<<" ns / call"<<endl;
 
    cout<<"Fast Transformation SRS speedup\t:"<< 1.*timer1.RealTime()/timer2.RealTime()*nCalls2/nCalls1<<endl;
